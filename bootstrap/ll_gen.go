...
func OLD_llModuleFrom(ast *Ast) LLModule {
	ret_mod := LLModule{
	...
	for i := range ast.defs {
		top_def := &ast.defs[i]
		switch ll_sth := OLD_llTopLevelFrom(&top_def.body, top_def, ast).(type) {
		case LLGlobal:
			ll_sth.anns.OLD_orig_ast_top_def = top_def
	...
	for i := range ret_mod.funcs {
		if fn_def := &ret_mod.funcs[i]; !fn_def.external {
			lit_curl_blocks := fn_def.anns.OLD_orig_ast_top_def.body.kind.(AstExprForm)[4].kind.(AstExprLitCurl)
			fn_def.basic_blocks = allocˇLLBasicBlock(len(lit_curl_blocks))
			for i := range lit_curl_blocks {
				fn_def.basic_blocks[i] = OLD_llBlockFrom(&lit_curl_blocks[i], fn_def.anns.OLD_orig_ast_top_def, ast, &ret_mod)
			}
			n_locals := 0
...
func OLD_llTopLevelFrom(expr *AstExpr, top_def *AstDef, ast *Ast) Any {
	...
	case AstExprForm:
		...
		if 1 == len(callee) {
			if astExprIsIdent(kwd, "global") {
				return OLD_llGlobalFrom(expr, top_def, ast)
			} else if astExprIsIdent(kwd, "declare") {
				return OLD_llFuncDeclFrom(expr, top_def, ast)
			} else if astExprIsIdent(kwd, "define") {
				return OLD_llFuncDefFrom(expr, top_def, ast)
			}
		}
	}
	return nil
...
func OLD_llGlobalFrom(full_expr *AstExpr, top_def *AstDef, ast *Ast) LLGlobal {
	...
	ret_global := LLGlobal{
		...
		ty:       OLD_llTypeFrom(astExprSlashed(&expr_form[1]), full_expr, ast),
	}
	if maybe_external != nil {
		...
		ret_global.initializer = OLD_llExprFrom(maybe_constant, ast, nil)
...
func OLD_llFuncDeclFrom(full_expr *AstExpr, top_def *AstDef, ast *Ast) LLFunc {
	expr_form := full_expr.kind.(AstExprForm)
	_ = expr_form[2].kind.(AstExprLitCurl)
	lit_curl_params := expr_form[4].kind.(AstExprLitCurl)
	ret_func := LLFunc{
		external:     true,
		ty:           OLD_llTypeFrom(astExprSlashed(&expr_form[3]), full_expr, ast),
	...
	for i := range lit_curl_params {
		...
		ret_func.params[i].ty = OLD_llTypeFrom(astExprSlashed(rhs), rhs, ast)
	}
	assert(len(ret_func.name) != 0)
	return ret_func
}

func OLD_llFuncDefFrom(full_expr *AstExpr, top_def *AstDef, ast *Ast) LLFunc {
	expr_form := full_expr.kind.(AstExprForm)
	...
	ret_func := LLFunc{
		name:         top_def.anns.name,
		external:     false,
		ty:           OLD_llTypeFrom(astExprSlashed(&expr_form[2]), &expr_form[2], ast),
		params:       allocˇLLFuncParam(len(lit_curl_params)),
		basic_blocks: nil,
	}
	for i := range lit_curl_params {
...
func OLD_llBlockFrom(pair_expr *AstExpr, top_def *AstDef, ast *Ast, ll_mod *LLModule) LLBasicBlock {
	...
	for i := range lit_clip_instrs {
		ret_block.instrs[i] = OLD_llInstrFrom(&lit_clip_instrs[i], ast, ll_mod)
...
func OLD_llInstrFrom(expr *AstExpr, ast *Ast, ll_mod *LLModule) LLInstr {
	switch it := expr.kind.(type) {
	case AstExprForm:
		...
		if 1 == len(callee) {
			...
			if astExprIsIdent(kwd, "ret") {
				assert(len(it) == 2)
				return LLInstrRet{expr: OLD_llExprFrom(&it[1], ast, ll_mod).(LLExprTyped)}
				...
				if maybe_form, _ := rhs.kind.(AstExprForm); len(maybe_form) == 2 {
					if slashed := astExprSlashed(&maybe_form[0]); len(slashed) == 1 {
						if ident, _ := slashed[0].kind.(AstExprIdent); len(ident) != 0 && ident[0] == 'I' {
							expr_ty := OLD_llExprFrom(rhs, ast, ll_mod).(LLExprTyped)
							instr = LLInstrBinOp{
								ty:      expr_ty.ty,
								lhs:     expr_ty.expr,
								rhs:     LLExprLitInt(0),
				...
				if instr == nil {
					instr = OLD_llInstrFrom(rhs, ast, ll_mod)
				}
				if instr == nil {
				...
				assert(len(it) == 3)
				load_expr := OLD_llExprFrom(&it[2], ast, ll_mod)
				ret_load := LLInstrLoad{ty: OLD_llTypeFrom(astExprSlashed(&it[1]), expr, ast)}
				ret_load.expr = LLExprTyped{expr: load_expr, ty: LLTypePtr{ty: ret_load.ty}}
				return ret_load
			} else if astExprIsIdent(kwd, "store") {
				assert(len(it) == 3)
				dst_expr := OLD_llExprFrom(&it[1], ast, ll_mod)
				ret_store := LLInstrStore{
					dst:  LLExprTyped{expr: dst_expr},
					expr: OLD_llExprFrom(&it[2], ast, ll_mod).(LLExprTyped),
				}
				...
				ret_call := LLInstrCall{
					ty:     OLD_llTypeFrom(astExprSlashed(&it[2]), expr, ast),
					callee: OLD_llExprFrom(&it[1], ast, ll_mod),
					args:   allocˇLLExprTyped(len(lit_clip_args)),
				}
				for i := range lit_clip_args {
					ret_call.args[i] = OLD_llExprFrom(&lit_clip_args[i], ast, ll_mod).(LLExprTyped)
				}
				return ret_call
				...
				ret_switch := LLInstrSwitch{
					comparee:           OLD_llExprFrom(&it[1], ast, ll_mod).(LLExprTyped),
				...
				for i := range lit_curl_cases {
					...
					ret_switch.cases[i].expr = OLD_llExprFrom(lhs, ast, ll_mod).(LLExprTyped)
				...
				ret_gep := LLInstrGep{
					ty:       OLD_llTypeFrom(astExprSlashed(&it[1]), expr, ast),
					base_ptr: OLD_llExprFrom(&it[2], ast, ll_mod).(LLExprTyped),
					indices:  allocˇLLExprTyped(len(lit_clip_idxs)),
				}
				for i := range lit_clip_idxs {
					ret_gep.indices[i] = OLD_llExprFrom(&lit_clip_idxs[i], ast, ll_mod).(LLExprTyped)
				}
				return ret_gep
				...
				assert(len(it) == 4)
				return LLInstrBrIf{
					cond:                OLD_llExprFrom(&it[1], ast, ll_mod),
				...
				for i := range lit_curl_preds {
					...
					ret_phi.predecessors[i] = LLPhiPred{
						...
						expr:       OLD_llExprFrom(rhs, ast, ll_mod),
					}
				...
				ret_alloca := LLInstrAlloca{
					ty:        OLD_llTypeFrom(astExprSlashed(&it[1]), expr, ast),
					num_elems: OLD_llExprFrom(&it[2], ast, ll_mod).(LLExprTyped),
				}
				return ret_alloca
				...
				ret_cmp := LLInstrCmpI{
					ty:  OLD_llTypeFrom(astExprSlashed(&it[2]), expr, ast),
					lhs: OLD_llExprFrom(&it[3], ast, ll_mod),
					rhs: OLD_llExprFrom(&it[4], ast, ll_mod),
				}
...
func OLD_llTypeFrom(expr_callee_slashed []*AstExpr, full_expr_for_err_msg *AstExpr, ast *Ast) LLType {
	...
	if ident[0] == 'P' && len(ident) == 1 {
		sub_type := OLD_llTypeFrom(expr_callee_slashed[1:], full_expr_for_err_msg, ast)
		if sub_type == nil {
		...
		sub_type := OLD_llTypeFrom(expr_callee_slashed[2:], full_expr_for_err_msg, ast)
		if sub_type != nil {
...
func OLD_llExprFrom(expr *AstExpr, ast *Ast, ll_mod *LLModule) LLExpr {
	switch it := expr.kind.(type) {
	case AstExprLitInt:
		...
		if len(slashed) != 0 {
			...
			if len(ident) != 0 {
				if len(it) == 2 && ident[0] >= 'A' && ident[0] <= 'Z' {
					ret_expr := LLExprTyped{ty: OLD_llTypeFrom(slashed, expr, ast), expr: OLD_llExprFrom(&it[1], ast, ll_mod)}
					return ret_expr
				}
			}
		}
		...
		if strEq(astNodeSrcStr(&it[0].base, ast), "/@") {
			...
			if ll_mod != nil {                // check whether referenced def is external
				...
				for i := 0; i < len(ll_mod.funcs) && !found; i++ {
					llf := &ll_mod.funcs[i]
					if llf.external && strEql(llf.anns.OLD_orig_ast_top_def.anns.name, name) {
						name = llf.name
						found = true
					}
				}
				for i := 0; i < len(ll_mod.globals) && !found; i++ {
					...
					if llg.external && strEql(llg.anns.OLD_orig_ast_top_def.anns.name, name) {
						name = llg.name
						break
					}
				}
	...
	if ret_instr := OLD_llInstrFrom(expr, ast, ll_mod); ret_instr != nil {
