...
define void @reverseBytes(i8* %buf, i64 %len) {
begin:
  %len_leq_1 = icmp ule i64 %len, 1
  br i1 %len_leq_1, label %end, label %prep
prep:
  %len_half = udiv i64 %len, 2
  %i_last = sub i64 %len, 1
  br label %loop
loop:
  %i = phi i64 [0, %prep], [%i_next, %loop]
...
b.1:
  ...
  call void @reverseBytes(i8* %buf, i64 %n)
